...
export default async function Page({ params: { budget } }: { params: { budget: string } }) {
    ...
    return (
        <>
            <div className="gap-2 flex flex-col lg:flex-row w-fit lg:w-full justify-between h-max items-center mb-14 sticky top-[81px] z-20 lg:top-[56px]">
                <div className="sticky top-[81px] z-20 lg:top-[56px] font-sans shadow-xl from-zinc-300 to-blue-200 bg-gradient-to-tr p-3 flex flex-col gap-2 rounded-lg  lg:self-baseline lg:w-[40%]">

                    <p className="text-2xl capitalize">price categories for budget</p>
                    <div className="flex justify-center text-3xl gap-4">
                        {data.map(([price], idx) =>
                            <a className="pr-2 border-r-2 border-black underline decoration-purple-500" key={idx * price} href={idx === 0 ? "#" : `#category-${price}`}>{price} NIS </a>
